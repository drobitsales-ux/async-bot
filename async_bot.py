...
def load_positions():
    ...
    try:
        ...
        stat_row = c.fetchone()
        if stat_row: daily_stats['pnl'], daily_stats['trades'], daily_stats['wins'], daily_stats['start_balance'] = stat_row
        conn.close()
    except: pass
...
async def send_tg_msg(text):
    ...
    try: async with global_session.post(url, json=payload) as resp: pass
    except: pass
...
async def update_macro_news():
    ...
    global NEWS_EVENTS
    # Здесь должен быть API вызов экономического календаря, пока используем эмуляцию
...
def is_news_spike():
    ...
    if not NEWS_EVENTS: return False
    now_str = datetime.now(timezone.utc).strftime("%H:%M")
    for ev in NEWS_EVENTS:
        if ev.get('time') == now_str: return True
    return False
...
def calculate_rsi(prices, window=14):
    ...
    avg_g = np.mean(gains[-window:]); avg_l = np.mean(losses[-window:])
    if avg_l == 0: return 100
    return 100 - (100 / (1 + avg_g / avg_l))

def get_pivots(highs, lows, order=5):
    """Чистый Python/NumPy (без scipy), устраняет проблему деплоя и ложных CHoCH"""
    h_idx, l_idx = [], []
    n = len(highs)
    for i in range(order, n - order):
        if all(highs[i] > highs[i-j] for j in range(1, order+1)) and all(highs[i] > highs[i+j] for j in range(1, order+1)): h_idx.append(i)
        if all(lows[i] < lows[i-j] for j in range(1, order+1)) and all(lows[i] < lows[i+j] for j in range(1, order+1)): l_idx.append(i)
    return h_idx, l_idx

def find_fvg(h, l, mode, lookback=10):
    """Истинный детектор FVG (разрыв между тенями 1-й и 3-й свечи)"""
    for i in range(1, min(lookback, len(h)-1)):
        idx = len(h) - 1 - i
        if mode == 'Long' and h[idx-1] < l[idx+1]: return {'top': l[idx+1], 'bottom': h[idx-1]}
        if mode == 'Short' and l[idx-1] > h[idx+1]: return {'top': l[idx-1], 'bottom': h[idx+1]}
    return None

async def process_smc_coin(sym):
    # 1. Проверка Киллзон и Новостей
    if not is_trade_session(): return None, 'out_of_session'
    if is_news_spike(): return None, 'news_spike'
    
    try: ohlcv = await exchange.fetch_ohlcv(sym, SMC_TIMEFRAME, limit=100)
    ...
    if recent_highs[-1] < recent_highs[-2] and current_price > recent_highs[-1]: mode = 'Long' 
    elif recent_lows[-1] > recent_lows[-2] and current_price < recent_lows[-1]: mode = 'Short' 
...
async def execute_trade(sym, signal):
    ...
    mode = signal['mode']; current_price = signal['price']
    
    if len(active_positions) >= MAX_POSITIONS: return
    if sum(1 for p in active_positions if p['direction'] == mode) >= MAX_POSITIONS_PER_DIRECTION: return
    
    # Оракулы
    has_volume = await check_global_volume_oracle(sym)
    if not has_volume: return
    ai_approved = await ask_gemini_oracle(sym, mode, current_price)
    if not ai_approved: return
...
async def monitor_positions_job():
    ...
    for pos in active_positions[:]:
        sym = pos['symbol']; is_long = pos['direction'] == 'Long'
        ...
        pos_side = 'LONG' if is_long else 'SHORT'; sl_side = 'sell' if is_long else 'buy'
        ...
        if curr:
            ...
            if pnl_pct >= 1.5 and not pos.get('be_moved'):
                ...
                try:
                    ...
                    pos['current_sl'] = new_sl; pos['sl_order_id'] = sl_ord['id']; pos['be_moved'] = True
                    save_positions(); await send_tg_msg(f"🛡 <b>{sym}</b>: SL перенесен в БУ ({new_sl})")
                except: pass
            ...
            if pnl_pct >= 2.5 and not pos.get('tp80_hit'):
                ...
                try:
                    ...
                    pos['sl_order_id'] = sl_ord['id']; save_positions()
                    await send_tg_msg(f"💰 <b>{sym}</b>: Зафиксировано 50% прибыли.")
                except: pass
        else:
            active_positions.remove(pos); save_positions()
            await send_tg_msg(f"🏁 <b>{sym}</b>: Позиция закрыта.")

class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self): self.send_response(200); self.end_headers(); self.wfile.write(b"BingX Async Bot v8.97 PROP + All Oracles Active")
    def log_message(self, format, *args): return 
def run_server(): HTTPServer(('0.0.0.0', int(os.environ.get('PORT', 10000))), HealthCheckHandler).serve_forever()
...
async def main():
    ...
    logging.info("🚀 Запуск BINGX ASYNC БОТА v8.97 (PROP + AI, Global Oracle & News Filter)...")
    await send_tg_msg("🟢 <b>BINGX ASYNC БОТ v8.97 PROP</b> запущен (Все баги устранены, Оракулы активны)")
