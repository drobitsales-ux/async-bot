...
# которая на форварде распалась до WR ~61% (см. BOT_SPEC.md §2.3) — тот же
# паттерн переобучения, что и с PB. Сегмент "RR входа": lt0.7 → 7 сд, WR 0%,
# Avg -2.54%, PF 0.00. P(0 побед из 7 | даже WR 40%) = 0.028 — не случайность.
...
async def rb_signal(sym: str, btc_ctx: dict):
    ...
    # [v53] fix off-by-one: срез [-49:-1] включал свип-свечу [-2] →
    # range_low <= lo2 всегда → условие пробоя невыполнимо.
...
async def single_asset_signal(btc_ctx: dict):
    ...
    # return НЕ меняются. Нужен заранее для shadow-логирования отсеянных
    # сетапов (см. ниже): нужно знать, был бы сетап валиден, если бы не
    # volume/RR фильтр.
    mode = None
    ...
    # Шорт: цена сильно ВЫШЕ VWAP + перекуплен → возврат вниз к VWAP
        ...
        mode = 'Short'
    # "Остальные условия выполнены" = направление есть И дистанция в
...
async def monitor_all():
    ...
    # [v51] КРИТИЧНО: process_pos() при закрытии зовёт save_all() ДО обрезки
    # списков (обрезка — только здесь, после цикла) → на диске остаётся
    # "призрак" уже закрытой позиции. Если процесс перезапустится (деплой)
    # до следующего save_all() из другого места — при рестарте load_all()
    # восстановит призрака, monitor_all() снова увидит live=None и
    # ПОВТОРНО засчитает закрытие: daily_stats['trades']/pnl_pct растут на
    # каждом рестарте за ОДНУ и ту же сделку (подтверждено: -0.678%×4≈-2.71%,
    # ровно как в логе circuit breaker). Финальный save_all() с уже
    # обрезанными списками закрывает это окно.
    save_all()
    ...
    try:
        ...
        for strat, emoji in [('PB', '🎯'), ('RB', '🎯'), ('SA_SHADOW', '🎯')]:
            ...
            if strat == 'PB':
                ...
                parts.append('  Объём:')
                parts += _feature(con, strat, 'vol_ratio',
                    [('<1.0', 0, 1.0), ('1.0-1.3', 1.0, 1.3), ('1.3-2.0', 1.3, 2.0)])
                parts.append('  RR входа:')
